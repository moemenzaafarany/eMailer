...
export const OrgUsersModel = eCreateContext(() => {
  const { UID } = useUserModel();
  const data = eUseState(null);

  const getCall = eUseAsyncState(async () => {
    let r = await ApisEB.call("admin/organization/users/get", {
      method: "POST",
      bodyData: {
        UID: UID,
      },
  ...
  const editCall = eUseAsyncState(async ({ item, formData }) => {
    ...
    let r = await ApisEB.call("admin/organization/users/update", {
      method: "POST",
      bodyData: formData,
    });
    if (r.success !== true) {
  ...
  const addCall = eUseAsyncState(async ({ formData }) => {
    ...
    formData.set("UID", UID);
    let r = await ApisEB.call("admin/organization/users/add", {
      method: "POST",
      bodyData: formData,
    });
    if (r.success !== true) {
  ...
  const deleteCall = eUseAsyncState(async ({ item }) => {
    ...
    formData.set("user_id", item?.id);
    formData.set("UID", UID);
    let r = await ApisEB.call("admin/organization/users/delete", {
      method: "POST",
      bodyData: formData,
    });
    if (r.success !== true) {
...
const useOrgUsersModel = OrgUsersModel.Use;
export default useOrgUsersModel;
